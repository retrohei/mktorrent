...
static char *get_torrent_name(char *target)
{
	char *p, *b;
	...
	printf("target: %s\n", target);
	b = strchr(p, '/');
	if (!b && (!finish && !daemon_start)) {
		fprintf(stderr, "error: --pa --paf only work with "
...
static char *get_dir_name(char *target)
{
	char *p1, *p2;
	
	p1 = strdup(target);
	p2 = strchr(p1, '/');
	if (!p2 && (!finish && !daemon_start)) {
		printf("bad target directory\n");
		exit(EXIT_FAILURE);
	}
	if (p2)
		*p2 = 0;
...
static char *get_path_name(char *target)
{
	char *p1, *p2;
...
static int is_dir(char *target)
{
	...
	file_list->path = get_path_name(target);
	file_list->size = s.st_size;
	file_list->next = NULL;
	file_list->pending = 1;
	file_list->hashed = 0;
	/* ..and size global variables */
	torrent_size = s.st_size;
...
static void print_help()
{
	...
	printf
	    ("Usage: mktorrent [OPTIONS] <target directory or filename>\n\n"
	     ...
	     "-x, --pa                : hash one file of the directory\n"
	     "-f, --paf               : create the final .torrent from the parts\n"
	     "-p, --private           : set the private flag\n"
	     "-r, --remove  <file>    : remove/unhash <file> from the torrent archive\n"
	     "-g, --logfile <logfile> : absolute path of the log file\n"
	...
	printf
	    ("Usage: mktorrent [OPTIONS] <target directory or filename>\n\n"
	     ...
	     "-r <file>    : remove/unhash <file> from the torrent archive\n"
	     "-g <logfile> : absolute path of the log file\n"
	     "-s,          : start the daemon\n"
	     "-x             : do partial hashing\n"
	     "-f             : create the final .torrent from the parts\n"
	     "-v             : be verbose\n"
	     "\nPlease send bug reports, patches, feature requests, praise and\n"
	     "general gossip about the program to: esmil@imf.au.dk\n");
#endif
...
void init(int argc, char *argv[])
{
	...
	const char *opt_string = "a:c:dhl:n:o:g:pvxfrs";
#ifndef NO_LONG_OPTIONS
	...
	if (torrent_name == NULL) {
		torrent_name = get_torrent_name(argv[optind]);
	}
	...
	if (partial)
		directory_name = get_dir_name(argv[optind]);
